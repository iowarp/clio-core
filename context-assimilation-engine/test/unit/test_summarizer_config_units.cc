 ...
 *
 * Pure unit tests (no CLIO runtime) for the header-only summarizer
 * config/task plumbing that the runtime-integration tests never exercise
 * directly:
 *
 *   - SummarizerConfig::LoadConfig — YAML parsing of next_pool_id,
 *     label_endpoint, label_prompts and label_matches (all sub-fields), plus
 *     the empty-config early-return and the malformed-YAML best-effort catch.
 *   - SummarizerConfig / LabelMatch serialize round-trips.
 *   - SummarizerConfig copy and compose-pool-id constructors.
 *
 * These paths live in summarizer_tasks.h and compile straight into this TU,
 * so no runtime, CTE client, or network is required.
 */
...
using clio::cae::summarizer::SummarizerConfig;
...
TEST_CASE("SummarizerConfig - LoadConfig empty config is a no-op",
          "[summarizer][config][loadconfig]") {
  SummarizerConfig params;
...
TEST_CASE("SummarizerConfig - LoadConfig parses next_pool_id",
          "[summarizer][config][loadconfig]") {
  SummarizerConfig params;
...
TEST_CASE("SummarizerConfig - LoadConfig ignores next_pool_id without a dot",
          "[summarizer][config][loadconfig]") {
  SummarizerConfig params;
...
TEST_CASE("SummarizerConfig - LoadConfig parses label_endpoint",
          "[summarizer][config][loadconfig]") {
  SummarizerConfig params;
...
TEST_CASE("SummarizerConfig - LoadConfig parses label_prompts map",
          "[summarizer][config][loadconfig]") {
  SummarizerConfig params;
  params.LoadConfig(MakePoolConfig(
      "label_prompts:\n"
...
TEST_CASE("SummarizerConfig - LoadConfig parses label_matches with all fields",
          "[summarizer][config][loadconfig]") {
  SummarizerConfig params;
  params.LoadConfig(MakePoolConfig(
      "label_matches:\n"
...
TEST_CASE("SummarizerConfig - LoadConfig label_matches uses field defaults",
          "[summarizer][config][loadconfig]") {
  SummarizerConfig params;
...
TEST_CASE("SummarizerConfig - LoadConfig parses a full combined config",
          "[summarizer][config][loadconfig]") {
  SummarizerConfig params;
  params.LoadConfig(MakePoolConfig(
      "next_pool_id: \"600.1\"\n"
...
TEST_CASE("SummarizerConfig - LoadConfig swallows malformed YAML",
          "[summarizer][config][loadconfig]") {
  SummarizerConfig params;
...
TEST_CASE("SummarizerConfig - copy constructor copies all fields",
          "[summarizer][config][ctor]") {
  SummarizerConfig src;
...
TEST_CASE("SummarizerConfig - compose pool-id constructor copies config",
          "[summarizer][config][ctor]") {
  SummarizerConfig src;
...
TEST_CASE("SummarizerConfig - serialize round-trips config", "[summarizer][config][serialize]") {
  SummarizerConfig in;
