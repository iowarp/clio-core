/**
 * Container virtual API dispatch for the checkpoint ChiMod (Run / Save / Load /
 * NewTask / Aggregate), switch-case over Method ids. Hand-maintained to
 * match clio_mod.yaml + checkpoint_methods.h (mirrors the replication chimod's
 * autogen/replication_lib_exec.cc).
 *
 ...
 * including the core's own kPutBlob/kGetBlob/kGetBlobSize/kMultiPutBlob ids
 * with the core's task structs — and every DEFAULT case delegates to the
 * core container, so any core method this module does not override behaves
...
// functions in sync.
#define CLIO_CHECKPOINT_FOR_EACH_METHOD(X)              \
...
#define X(MID, TASK, HANDLER)                                       \
    ...
    CLIO_CHECKPOINT_FOR_EACH_METHOD(X)
#undef X
    default:
...
#undef CLIO_CHECKPOINT_FOR_EACH_METHOD
