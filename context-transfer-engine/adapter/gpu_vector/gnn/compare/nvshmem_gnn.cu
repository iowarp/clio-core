/* NVSHMEM multi-GPU GNN feature-store readout (Track B). Shard the N x 128 feature
 * matrix uncompressed across N GPUs; sum-pool locally; reduce the 128-d partial
 * sums via an NVSHMEM collective over the symmetric heap. Ceiling = sum HBM (1.0x). */
#include <cstdio>
#include <cstdlib>
#include <cstdint>
#include <vector>
...
int main(int argc,char** argv){
  ...
  double gib=argc>1?atof(argv[1]):8.0; int iters=argc>2?atoi(argv[2]):3;
  const char* data=argc>3?argv[3]:"/u/rpawar/data/sift/sift/sift_base.fvecs";
  CK(cudaSetDevice(rank%8)); MPI_Comm mc=MPI_COMM_WORLD;
  nvshmemx_init_attr_t attr=NVSHMEMX_INIT_ATTR_INITIALIZER; attr.mpi_comm=&mc;
  ...
  if(!all){ if(!rank) std::fprintf(stderr,"[NVK][RESULT] backend=nvshmem ngpu=%d gib_per_gpu=%.1f pooled_gib=%.1f ran=OOM\n",np,gib,gib*np);
    cudaGetLastError(); nvshmem_finalize(); MPI_Finalize(); return 0; }
  ...
  double* s=(double*)nvshmem_malloc(kDims*sizeof(double));
  double* r=(double*)nvshmem_malloc(kDims*sizeof(double)); std::vector<double> hs(kDims);
  MPI_Barrier(MPI_COMM_WORLD); auto t0=std::chrono::steady_clock::now();
  for(int it=0; it<iters; ++it){ CK(cudaMemset(s,0,kDims*sizeof(double)));
    int th=256; unsigned long long bl=(n_local+th-1)/th;
    AggKernel<<<(unsigned)std::min(bl,(unsigned long long)2147483647u),th>>>(d,n_local,s); CK(cudaDeviceSynchronize());
    nvshmem_double_sum_reduce(NVSHMEM_TEAM_WORLD,r,s,kDims); nvshmem_barrier_all();
  }
  MPI_Barrier(MPI_COMM_WORLD); double dt=std::chrono::duration<double>(std::chrono::steady_clock::now()-t0).count();
  CK(cudaMemcpy(hs.data(),r,kDims*sizeof(double),cudaMemcpyDeviceToHost));
  ...
  if(!rank) std::fprintf(stderr,"[NVK][RESULT] backend=nvshmem ngpu=%d gib_per_gpu=%.1f pooled_gib=%.1f ran=ran iters=%d time_s=%.3f agg_mibps=%.1f peak_gpu_mib_per_gpu=%lld checksum=%.6e\n",
    np,gib,pooled/1024.0,iters,dt,dt>0?pooled*iters/dt:0.0,mx,hs[0]);
  nvshmem_free(s);nvshmem_free(r);cudaFree(d); nvshmem_finalize(); MPI_Finalize(); return 0;
}
