...
/**
 * F16 transaction-driven weight GEMV demo / benchmark.
 *
 * Exercises gpu_vector_gemv.h: a half-precision weight matrix W[n_rows,n_cols]
 * is stored in a Vector<__half> (paged HBM -> pinned DRAM -> CTE blob storage)
 * and y = W * x is computed by streaming each weight row through
 * dev::vector::read_range. Weight residency is handled entirely by the
 * vector's GPU-resident cache + on-GPU cache-manager (rescore/prefetch) — no
 * CPU-mediated UVM fault on the hit path.
 *
 * Oversubscribe (weights > HBM cache) with --pages-per-block small and
 * --host-pages-per-block > 0 to engage the two-tier async cache manager and
 * see the GPU-driven eviction/prefetch machinery actually run.
 *
 * Verification: W[i] = (i % 7) (exact in f16), x[c] = 1
 *               => y[row] = sum_c ((row*n_cols + c) % 7).
 *
 *   --blocks N              Cache blocks == parallel row stripes (default 8)
 *   --rows-per-block R      Output rows per block (default 16)
 *   --n-cols K              Reduction dim / weights per row (default 2048)
...
#include <clio_cte/gpu_vector/gpu_vector_gemv.h>
...
/** Populate the weight matrix in the Vector: W[i] = __float2half(i % 7). */
__global__ void WriteWeightsF16Kernel(clio::run::IpcManagerGpuInfo info,
                                      gv::DeviceView<__half> view,
                                      clio::run::u64 elems_per_block) {
  CLIO_GPU_INIT(info, /*ipc_ptr=*/nullptr);
  dev::vector<__half> v(view, g_ipc_manager_ptr);
  ...
  v.write_range(lo, hi, [] (clio::run::u64 i) {
    return __float2half(static_cast<float>(i % 7));
  });
  (void)g_ipc_manager;
}
...
void EnsureInit(clio::run::u64 bdev_capacity_bytes) {
  ...
  auto bdev_create = bdev_client.AsyncCreate(
      clio::run::PoolQuery::Dynamic(), std::string("weight_gemv_ram"),
  ...
  auto reg_task = cte_client->AsyncRegisterTarget(
      "weight_gemv_ram", clio::run::bdev::BdevType::kRam,
...
int main(int argc, char *argv[]) {
  ...
  if (o.nblocks == 0 || o.rows_per_block == 0 || o.n_cols == 0 ||
      o.page_size % sizeof(__half) != 0) {
    std::fprintf(stderr, "invalid geometry (page_size must be a multiple of 2)\n");
    return 2;
  }
  ...
  const clio::run::u64 total_bytes = total_elems * sizeof(__half);
  const clio::run::u64 cache_bytes =
      static_cast<clio::run::u64>(o.nblocks) * o.pages_per_block * o.page_size;
  ...
  std::fprintf(stderr,
               "[GEMV] blocks=%u rows/block=%u n_cols=%llu -> n_rows=%llu\n"
               "[GEMV] weights(f16)=%llu B  HBM cache=%llu B  ratio=%.2fx\n"
               "[GEMV] page=%llu B pages/block=%u host_pages=%u mode=%s\n",
               ...
               total_bytes / static_cast<double>(cache_bytes),
               (unsigned long long)o.page_size, o.pages_per_block,
               o.host_pages_per_block,
               o.host_pages_per_block > 0 ? "kAsync(two-tier)" : "kLegacy");
  ...
  gv::Vector<__half> W("weight_gemv", o.nblocks, o.gpu_id, o.pages_per_block,
  ...
  WriteWeightsF16Kernel<<<o.nblocks, 32, 0, stream>>>(gpu_info, view,
                                                     elems_per_block);
  ...
  // 3. Run the F16 weight-application transaction: y = W * x.
  auto t0 = std::chrono::steady_clock::now();
  gv::LaunchWeightGemvF16(W, o.nblocks, o.gpu_id, dx, dy, o.n_cols,
  ...
  // 4. Verify against y[row] = sum_c ((row*n_cols + c) % 7).
  ...
  for (clio::run::u64 row = 0; row < n_rows; ++row) {
    ...
    if (static_cast<double>(hy[row]) != expect) {
      if (mismatches < 8) {
        std::fprintf(stderr, "[GEMV] mismatch row %llu: got %.1f want %.1f\n",
  ...
  double gflop = 2.0 * static_cast<double>(total_elems) / 1e9;
  std::fprintf(stderr, "[GEMV] gemv kernel: %lld us over %llu rows (%.2f GFLOP)\n",
               (long long)us, (unsigned long long)n_rows, gflop);
  if (mismatches == 0) {
    std::fprintf(stderr, "[GEMV] PASS: all %llu rows correct\n",
                 (unsigned long long)n_rows);
  } else {
    std::fprintf(stderr, "[GEMV] FAIL: %llu / %llu rows wrong\n",
