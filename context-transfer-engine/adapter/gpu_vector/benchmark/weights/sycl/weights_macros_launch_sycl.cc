...
#include "../weights_macros_launch.h"

#include "../weights_macros_kernels.h"
...
void LaunchSeed(dim3 grid, dim3 block, const GpuInfo &info, DevU32 v, u64 per,
  ...
  SubmitYieldable(grid, block, v, vw, sv, [=](DevU32 dev, u32 blk) {
    SeedLaneMacro(dev, per, page_elems, flat_pct, base_idx, blk);
  });
}
...
void LaunchWeights(dim3 grid, dim3 block, const GpuInfo &info, DevU32 v,
  ...
  (void)info;
  SubmitYieldable(grid, block, v, vw, sv, [=](DevU32 dev, u32 blk) {
    WeightsLaneMacro(dev, per, page_elems, sum, page_sum, page_visits,
                           base_idx, blk);
  });
}
