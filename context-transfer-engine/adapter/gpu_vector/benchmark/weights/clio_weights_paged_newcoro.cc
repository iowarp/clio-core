 ...
 */
CTP_GPU_FUN CLIO_COROC_INLINE void SeedLaneCoro(gv::DeviceVector<::clio::run::u32> v,
                                      ::clio::run::u64 per,
...
CTP_GPU_FUN CLIO_COROC_INLINE void WeightsLaneCoro(gv::DeviceVector<::clio::run::u32> v,
                                         ::clio::run::u64 per,
                                         ::clio::run::u64 page_elems,
  ...
  for (::clio::run::u64 off = 0; off < per; off += page_elems) {
    CO_AWAIT(v.CoFetch(0, base + off,
                     (off + page_elems <= per) ? page_elems : (per - off)));
    auto h = CO_AWAIT(v.CoHoldPage(
        base + off, (off + page_elems <= per) ? page_elems : (per - off), /*write=*/false));
    const ::clio::run::u64 n =
        (off + page_elems <= per) ? page_elems : (per - off);
...
void LaunchSeed(dim3 grid, dim3 block, const GpuInfo &info, DevU32 v, u64 per,
  ...
  SubmitYieldable(grid, block, v, vw, sv, [=](clio::co::Ctx &_cy, DevU32 dev, u32 blk) {
    SeedLaneCoro(_cy, dev, per, page_elems, flat_pct, base_idx, blk);
  });
}
...
void LaunchWeights(dim3 grid, dim3 block, const GpuInfo &info, DevU32 v,
  ...
  (void)info;
  SubmitYieldable(grid, block, v, vw, sv, [=](clio::co::Ctx &_cy, DevU32 dev, u32 blk) {
    WeightsLaneCoro(_cy, dev, per, page_elems, sum, page_sum, page_visits,
                           base_idx, blk);
  });
}
...
__global__ GV_LAUNCH_BOUNDS void SeedKernel(GpuInfo info, DevU32 v, u64 per,
  ...
  v.Init(yv.Block());
  __syncthreads();
  CLIO_COROC_RUN(yv, ys, SeedLaneCoro(_cy, v, per, page_elems, flat_pct, base_idx,
                              yv.Block()));
}
...
__global__ GV_LAUNCH_BOUNDS void WeightsKernel(GpuInfo info, DevU32 v, u64 per, u64 page_elems,
  ...
  v.Init(yv.Block());
  __syncthreads();
  CLIO_COROC_RUN(yv, ys, WeightsLaneCoro(_cy, v, per, page_elems, sum, page_sum,
