...
def main():
    ...
    logs += sorted(glob.glob(os.path.join(D, 'rerun_*_*.log')) + glob.glob(os.path.join('/lus/flare/projects/IOWarp/llogan_e1', 'scale_*_*.log')) + glob.glob(os.path.join('/lus/flare/projects/IOWarp/llogan_e1', 'rerun_*_*.log')))
    ranks, cells = {}, {}
    for log in logs:
    ...
    with open(os.path.join('/lus/flare/projects/IOWarp/llogan_e1', 'e1_ranks.csv'), 'w', newline='') as f:
        w = csv.DictWriter(f, keys); w.writeheader(); w.writerows(rk)
    cl = sorted(cells.values(), key=lambda x: (x['nodes'], x['workload'], x['substrate']))
    with open(os.path.join('/lus/flare/projects/IOWarp/llogan_e1', 'e1_cells.csv'), 'w', newline='') as f:
