...
OUT = sys.argv[1] if len(sys.argv) > 1 else '/lus/flare/projects/IOWarp/llogan_e1/figs'
...
def parse():
    ...
    logs += sorted(glob.glob(os.path.join(D, 'rerun_*_*.log')) + glob.glob(os.path.join('/lus/flare/projects/IOWarp/llogan_e1', 'scale_*_*.log')) + glob.glob(os.path.join('/lus/flare/projects/IOWarp/llogan_e1', 'rerun_*_*.log')))  # override
    for log in logs:
