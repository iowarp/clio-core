...
/**
 * MLP training, NVSHMEM edition: the same model-parallel decomposition as
 * the MPI sibling with the weight shards and activation buffers ON THE
 * SYMMETRIC HEAP. NVSHMEM's host collectives refuse limited-MPG runs, so
 * under an MPI bootstrap the gathers ride MPI host-staged (the family-wide
 * dodge); on a one-PE-per-GPU cluster the symmetric residency is what a
 * fcollect-based variant would exploit. A rank owns h-rows of W1 and o-rows of W2 in plain device memory;
 ...
 *
 * Run recipe: mpirun -n 4 clio_lbann_nvshmem_bench --hidden 4096 --steps 5
 */
...
int main(int argc, char **argv) {
  ...
  MPI_Init(&argc, &argv);
  {
    ...
    LB_CUDA_CHECK(cudaSetDevice(ndev > 0 ? wr % ndev : 0));
  ...
  const int nranks = nvshmem_n_pes();
  ...
  if (H % nranks != 0 || O % nranks != 0) {
    if (rank == 0) std::fprintf(stderr, "LBANN NVSHMEM: H and O must divide the "
                                "rank count\n");
    nvshmem_finalize();
#if defined(MD_NVSHMEM_USE_MPI)
    MPI_Finalize();
  ...
  if (rank == 0) {
    std::printf("MLP training, NVSHMEM edition: %llu -> %llu -> %llu, batch=%llu"
  ...
  float *d_w1 = static_cast<float *>(nvshmem_malloc(hper * I *
                                                    sizeof(float)));
  float *d_b1 = static_cast<float *>(nvshmem_malloc(hper * sizeof(float)));
  float *d_w2 = static_cast<float *>(nvshmem_malloc(oper * H *
                                                    sizeof(float)));
  float *d_b2 = static_cast<float *>(nvshmem_malloc(oper * sizeof(float)));
  if (!d_w1 || !d_b1 || !d_w2 || !d_b2) {
  ...
  if (rank == 0) {
    ...
    std::printf("%s\n", rc == 0 ? "LBANN NVSHMEM: ALL GATES PASS"
                                : "LBANN NVSHMEM: GATE FAILURE");
  }
#if defined(MD_NVSHMEM_USE_MPI)
  MPI_Bcast(&rc, 1, MPI_INT, 0, MPI_COMM_WORLD);
#endif
  nvshmem_free(d_w1); nvshmem_free(d_b1);
  nvshmem_free(d_w2); nvshmem_free(d_b2);
  nvshmem_finalize();
#if defined(MD_NVSHMEM_USE_MPI)
  MPI_Finalize();
#endif
