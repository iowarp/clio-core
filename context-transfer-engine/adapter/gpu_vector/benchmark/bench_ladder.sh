...
# the checksum equals rung 1's (paging must be invisible to the result); an
...
case "${wl}" in
  ...
  grayscott)
    ...
    resident_slots=64; ooc_slots=8 ;;
  *) echo "bench_ladder: unknown workload ${wl}"; exit 2 ;;
esac
# 256 blocks x 1 MB pages: <pernode> MB resident needs pernode/256 slots.
resident_slots=$(( pernode / 256 + 8 ))
...
rung() {  # rung <k> <ranks> <slots> <label>
  local k=$1 n=$2 slots=$3 label=$4 r rc cs cs0 ev
  local dir="${workdir}/rung${k}"  # separate: a single local expands before assigning
  echo "=== rung ${k}: ${n} rank(s), ${label}, ${pernode} MB/node, ${slots} slots/block"
  if ! bash "${here}/run_colocated.sh" "${n}" "${dir}" "${exe}" $(args $(( pernode * n )) "${slots}") > "${dir}.out" 2>&1; then
  ...
  if [ -n "${LADDER_CS:-}" ] && [ "${cs0}" != "${LADDER_CS}" ]; then
    echo "FAIL rung ${k}: checksum ${cs0} != rung 1's ${LADDER_CS} (paging changed the answer)"; return 1
  fi
  LADDER_CS=${cs0}
