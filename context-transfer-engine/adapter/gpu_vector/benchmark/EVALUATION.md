...
| kmeans | the same, baseline assignment tiled per work-group (local-memory sums and counts, one global atomic per group per entry) | rerun queued | rerun queued | rerun queued |
