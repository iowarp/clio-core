...
/**
 * PME spread+gather, NVSHMEM edition: the mesh is cut into z-plane slabs, one
 ...
 *
 * Run recipe: mpirun -n 4 clio_gmx_nvshmem_bench --k 128 --atoms 200000
 */
...
int main(int argc, char **argv) {
  ...
  MPI_Init(&argc, &argv);
  {
    ...
    GX_CUDA_CHECK(cudaSetDevice(ndev > 0 ? wr % ndev : 0));
  ...
  if (rank == 0) {
    std::printf("PME spread+gather, NVSHMEM edition: mesh=%llu^3, atoms=%llu, "
                "%d ranks\n",
  ...
  // Equal slabs only: the symmetric heap hands every PE one allocation.
  if (K % nranks != 0) {
    if (rank == 0) std::fprintf(stderr, "GMX NVSHMEM: K %% PEs != 0\n");
    return 2;
  }
  unsigned long long *d_out = nullptr;
  unsigned long long *d_slab = static_cast<unsigned long long *>(
      nvshmem_malloc((z1 - z0) * plane * sizeof(unsigned long long)));
  if (!d_slab) { std::fprintf(stderr, "nvshmem_malloc failed\n"); return 1; }
  ...
  if (rank == 0) {
    ...
    std::printf("%s\n", rc == 0 ? "GMX NVSHMEM: ALL GATES PASS"
                                : "GMX NVSHMEM: GATE FAILURE");
  }
#if defined(MD_NVSHMEM_USE_MPI)
