...
#PBS -j oe
#
# Run ONE ported paged benchmark across FOUR Aurora nodes (debug-scaling),
# one rank per node. The two-node script with the node count taken from the
# allocation; everything else is identical.
#
...
RUNDIR=${RUNDIR:-${ROOT}/build-spike/run4n_${BENCH_NAME}}
...
cd "${RUNDIR}"
ulimit -c unlimited
rm -f ./rank*.log ./hostfile ./clio_4n.yaml
...
export BENCH_RANK_CAP="${BENCH_CAP:-240}"
...
echo "--- run (${BENCH_CAP:-240}s cap per rank, ${NRANKS} ranks, --cpu-bind ${CPU_BIND}) ---"
start=$SECONDS
...
mpiexec -n "${NRANKS}" --ppn 1 --envall --cpu-bind "${CPU_BIND}" bash -c '
  ...
  cd "$BENCH_RANK_DIR"
  timeout --signal=TERM --kill-after=10s ${BENCH_RANK_CAP:-240} \
...
case "${rc}" in
  ...
  124) echo "RESULT ${BENCH_NAME}x${NRANKS}: TIMEOUT (a rank exceeded the cap)" ;;
