...
/* GV_MD_CORO means "this build has an MD device implementation". md_common.h
 * only ever sets it for the coroutine one, because that was the only one there
 * was -- so the whole host driver below, and main() itself, are gated on it.
 * This edition supplies the MACRO implementation, which is equally real and is
 * the one that compiles for SPIR-V, so assert it here rather than let the
 * driver compile itself out and exit 2 at runtime. */
#ifndef GV_MD_CORO
#define GV_MD_CORO 1
#endif
...
namespace md = ::clio::gv_bench::md_macros;
