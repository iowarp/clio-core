...
#include "md_macros_launch.h"

#include "md_macros_kernels.h"

#include <clio_ctp/util/gpu_api.h>

#include <sycl/sycl.hpp>

namespace clio::gv_bench::md_macros {
...
void LaunchReadProbe(dim3 grid,
  ...
  Submit(grid, block, [=]() {
    ...
    ReadProbeMacro(x_, passes, nblocks, yv.Block());
  });
}
...
void LaunchPublishSlab(dim3 grid,
  ...
  Submit(grid, block, [=]() {
    ...
    // what CLIO_YCORO_RUN's prologue does for the coroutine edition.
    CLIO_YKERNEL_ENTER(yv, ys);
    PublishSlabMacro(x_, v_, nb, cap, z0, z1, gen, halo_first, nblocks, yv.Block());
  });
}
...
void LaunchHaloPin(dim3 grid,
  ...
  Submit(grid, block, [=]() {
    ...
    // what CLIO_YCORO_RUN's prologue does for the coroutine edition.
    CLIO_YKERNEL_ENTER(yv, ys);
    HaloPinMacro(x_, nb, cap, z0, z1, gen, yv.Block());
  });
}
...
void LaunchRefaultWrite(dim3 grid,
  ...
  Submit(grid, block, [=]() {
    ...
    // what CLIO_YCORO_RUN's prologue does for the coroutine edition.
    CLIO_YKERNEL_ENTER(yv, ys);
    RefaultWriteMacro(x_, pg_lo, pg_hi, round, ppp, nblocks, yv.Block());
  });
}
...
void LaunchRefaultVerify(dim3 grid,
  ...
  Submit(grid, block, [=]() {
    ...
    // what CLIO_YCORO_RUN's prologue does for the coroutine edition.
    CLIO_YKERNEL_ENTER(yv, ys);
    RefaultVerifyMacro(x_, pg_lo, pg_hi, round, ppp, below_pg, above_pg, nblocks, yv.Block(), d_out);
  });
}
...
void LaunchHaloUnpin(dim3 grid,
  ...
  Submit(grid, block, [=]() {
    ...
    // what CLIO_YCORO_RUN's prologue does for the coroutine edition.
    CLIO_YKERNEL_ENTER(yv, ys);
    HaloUnpinMacro(x_, nb, cap, z0, z1, yv.Block());
  });
}
...
void LaunchThermo(dim3 grid,
  ...
  Submit(grid, block, [=]() {
    ...
    // what CLIO_YCORO_RUN's prologue does for the coroutine edition.
    CLIO_YKERNEL_ENTER(yv, ys);
    ThermoMacro(x_, v_, out, nb, cap, z0, z1, nblocks, yv.Block());
  });
}
...
void LaunchForce(dim3 grid,
  ...
  Submit(grid, block, [=]() {
    ...
    // what CLIO_YCORO_RUN's prologue does for the coroutine edition.
    CLIO_YKERNEL_ENTER(yv, ys);
    ForceMacro(x_, f_, nb, cap, box, cutoff, eflag, acc, z0, z1, nblocks, yv.Block(), hgen, force_all);
  });
}
...
void LaunchBuildList(dim3 grid,
  ...
  Submit(grid, block, [=]() {
    ...
    // what CLIO_YCORO_RUN's prologue does for the coroutine edition.
    CLIO_YKERNEL_ENTER(yv, ys);
    BuildListMacro(x_, nl_, nb, cap, box, rlist, maxneigh, d_cnt, d_err, rowchunk, z0, z1, nblocks, yv.Block(), hgen);
  });
}
...
void LaunchListForce(dim3 grid,
  ...
  Submit(grid, block, [=]() {
    ...
    // what CLIO_YCORO_RUN's prologue does for the coroutine edition.
    CLIO_YKERNEL_ENTER(yv, ys);
    ListForceMacro(x_, f_, nl_, nb, cap, box, cutoff, maxneigh, d_cnt, eflag, acc, nocompute, rowchunk, z0, z1, nblocks, yv.Block(), hgen, force_all, band);
  });
}
...
void LaunchRebinWrap(dim3 grid,
  ...
  Submit(grid, block, [=]() {
    ...
    // what CLIO_YCORO_RUN's prologue does for the coroutine edition.
    CLIO_YKERNEL_ENTER(yv, ys);
    RebinWrapMacro(x_, nb, cap, box, z0, z1, nblocks, yv.Block());
  });
}
...
void LaunchRebinAssign(dim3 grid,
  ...
  Submit(grid, block, [=]() {
    ...
    // what CLIO_YCORO_RUN's prologue does for the coroutine edition.
    CLIO_YKERNEL_ENTER(yv, ys);
    RebinAssignMacro(x_, nb, cap, box, bincnt, d_dest, d_err, z0, z1, nblocks, yv.Block(), hgen);
  });
}
...
void LaunchGather(dim3 grid,
  ...
  Submit(grid, block, [=]() {
    ...
    // what CLIO_YCORO_RUN's prologue does for the coroutine edition.
    CLIO_YKERNEL_ENTER(yv, ys);
    GatherMacro(src_, srcx_, dst_, nb, cap, d_dest, keep_w, z0, z1, nblocks, yv.Block(), hgen);
  });
}
...
void LaunchSentinel(dim3 grid,
  ...
  Submit(grid, block, [=]() {
    ...
    // what CLIO_YCORO_RUN's prologue does for the coroutine edition.
    CLIO_YKERNEL_ENTER(yv, ys);
    SentinelMacro(dst_, nb, cap, z0, z1, nblocks, yv.Block());
  });
}
...
void LaunchMDIntegrate(dim3 grid,
  ...
  Submit(grid, block, [=]() {
    ...
    // what CLIO_YCORO_RUN's prologue does for the coroutine edition.
    CLIO_YKERNEL_ENTER(yv, ys);
    MDIntegrateMacro(x_, v_, f_, dt, drift, nb, cap, z0, z1, nblocks, yv.Block());
  });
}
