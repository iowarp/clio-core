 ...
 *
 * The same melt deck, the same bin-major padded layout, the same slab
 * decomposition and the same four gates as clio_lammps_md_mpi_bench -- with
 * the ONE variable changed that this row exists to isolate: how the device
 * side is EXPRESSED. Kokkos is not another transport (the halo still crosses
 * two-sided MPI, host-staged, exactly as in the MPI sibling); it is another
 * PROGRAMMING MODEL, one source with the backend chosen at build time. It is
 * what a portability-minded MD code would reach for instead of the paged
 * vector, so it is what the vector should be measured against.
 *
 * THE TRANSLATION, and what was deliberately NOT idiomatised
 * ---------------------------------------------------------
 * The CUDA block becomes a Kokkos TEAM: league_rank/team_rank/team_barrier
 * for blockIdx/threadIdx/__syncthreads, team_scratch for `extern __shared__`,
 * Kokkos::atomic_add for atomicAdd. The kernels that cooperate within a block
 * -- force, list build, thermo -- are TeamPolicy; the ones that are pure
 * per-slot work -- rebin, scatter, unpack, integrate -- are a flat
 * RangePolicy, which is what their grid-stride loops always were.
 *
 * Two things are kept unidiomatic ON PURPOSE, because the gates compare this
 * row's step-0 potential energy and its EXACT pair count against the other
 * substrates:
 *   - the in-team tree reduction is translated literally instead of becoming
 *     a Kokkos team reduce, so the summation order inside a team is
 *     unchanged;
 *   - the pair loops keep an explicit round-to-nearest FMA (Fmaf) rather than
 *     letting the compiler contract as it likes.
 * Anything looser would show up in the gates as drift that has nothing to do
 * with the substrate under test.
 *
 * Device memory is Kokkos' (kokkos_malloc / deep_copy / fence) but is handed
 * to the kernels as raw pointers, because the bin-major padded layout is
 * indexed with hand-computed offsets that a View would only obscure -- and
 * keeping them raw is what lets the pair loops stay line-for-line identical
 * to the siblings.
 *
 * `--blocks` and `--threads` keep their meaning: they are the league size and
 * the team size, so the same flags describe the same decomposition here as in
 * the MPI sibling.
 *
 * Like every baseline in this directory it links NOTHING from clio.
 *
 * Run recipe (identical flags to the MPI sibling):
 *   mpirun -n 1 clio_lammps_md_kokkos_bench --md --lattice 20 --steps 20 \
 *       --rebin 10 --temp 3.0 --cap 48 --blocks 128 --threads 128
 *   mpirun -n 2 clio_lammps_md_kokkos_bench --md --lattice 20 ...
 */
...
// ---------------------------------------------------------------------------
// The device side, in Kokkos.
//
// The CUDA block becomes a Kokkos TEAM, and that mapping is what the whole
// translation rests on:
//
//     blockIdx.x      ->  team.league_rank()
//     gridDim.x       ->  team.league_size()
//     threadIdx.x     ->  team.team_rank()
//     blockDim.x      ->  team.team_size()
//     __syncthreads() ->  team.team_barrier()
//     extern __shared__ -> team_scratch(0) (an unmanaged scratch View)
//     atomicAdd       ->  Kokkos::atomic_add
//
// The pair arithmetic, the minimum image, the padded-list entry packing and
// the in-team tree reduction are translated LINE FOR LINE rather than
// idiomatised. That is deliberate: the gates compare this row's step-0 PE and
// exact pair count against the other substrates, so anything that reassociates
// a float sum would show up as drift that has nothing to do with the
// substrate. In particular the tree reduction is kept instead of being
// replaced by a Kokkos team reduce, because it fixes the summation ORDER
// within a team.
// ---------------------------------------------------------------------------

using ExecSpace = Kokkos::DefaultExecutionSpace;
using TeamPol = Kokkos::TeamPolicy<ExecSpace>;
using Team = TeamPol::member_type;
using ScratchSpace = ExecSpace::scratch_memory_space;
template <typename T>
using Scratch = Kokkos::View<T *, ScratchSpace, Kokkos::MemoryUnmanaged>;

/** Round-to-nearest FMA, the same instruction the CUDA siblings emit. Kept
 *  explicit so no compiler is free to contract these particular products
 *  differently -- the integrator's bit-exactness rides on it. */
KOKKOS_INLINE_FUNCTION float Fmaf(float a, float b, float c) {
...
/** Offset of global plane `bz` within this rank's EXTENDED slab. */
KOKKOS_INLINE_FUNCTION u64 PlaneOff(const Decomp &d, u32 bz, u64 plane_elems) {
  const u32 rel = bz - d.z0;
  if (rel < d.nplanes) return static_cast<u64>(rel + 1) * plane_elems;
  if (bz == d.halo_lo) return 0;
  return static_cast<u64>(d.nplanes + 1) * plane_elems;
}

/**
 * Resolve the up-to-six contiguous row spans the stencil of rows
 * [y0-1, ylast+1] needs, across the three z-planes. Every thread computes the
 * identical answer into its own registers, exactly as the CUDA sibling does.
 */
KOKKOS_INLINE_FUNCTION int ResolveStencil(const Team &team, const Decomp &d,
                                          const float *x, u32 bz, u32 y0,
                                          u32 ylast, u64 row_elems,
                                          u64 plane_elems, const float **sp,
                                          u32 *sbase, u32 *scnt, u32 *sdz) {
  const u32 nb = d.nb;
  int nspans = 0;
  for (int dz = -1; dz <= 1; ++dz) {
...
/** Publish the nine stencil-row pointers for row `by` into team scratch. */
KOKKOS_INLINE_FUNCTION void PublishRow(const Team &team,
                                       const float **s_qptr, u32 by, u32 nb,
...
/** The in-team tree reduction of the CUDA siblings, kept verbatim so the
 *  summation order inside a team is identical. One atomic per team per
 *  quantity, exactly as before. */
KOKKOS_INLINE_FUNCTION void TeamReduceAdd(const Team &team, Scratch<double> red,
  ...
  for (int q = 0; q < nvals; ++q) {
    red[tr] = vals[q];
    team.team_barrier();
    for (u32 wd = ts / 2; wd > 0; wd >>= 1) {
      if (tr < wd) red[tr] += red[tr + wd];
      team.team_barrier();
    }
    if (tr == 0) Kokkos::atomic_add(&acc[q], red[0]);
    team.team_barrier();
  }
}

/** K3 cell-direct. Pair arithmetic copied verbatim from the MPI sibling. */
struct ForceFunctor {
  Decomp d;
  const float *x;
  float *f;
  ...
  KOKKOS_INLINE_FUNCTION void operator()(const Team &team) const {
    const u32 ts = static_cast<u32>(team.team_size());
    Scratch<double> red(team.team_scratch(0), ts);
    const float **s_qptr = static_cast<const float **>(
        team.team_scratch(0).get_shmem(9 * sizeof(const float *)));
    const u32 tr = static_cast<u32>(team.team_rank());
    ...
    for (u64 ch = team.league_rank(); ch < nchunks; ch += team.league_size()) {
      ...
      const int nspans = ResolveStencil(team, d, x, bz, y0, ylast, row_elems,
                                        plane_elems, sp, sbase, scnt, sdz);

      for (u32 by = y0; by <= ylast; ++by) {
        PublishRow(team, s_qptr, by, nb, nspans, sp, sbase, scnt, sdz,
                   row_elems);
        float *const fp = f + (static_cast<u64>(lz) * nb + by) * row_elems;
        for (u64 e = tr; e < row_elems; e += ts) fp[e] = 0.0f;
        team.team_barrier();
        const float *const ip_row = s_qptr[4];
        for (u64 s = tr; s < islots; s += ts) {
          ...
          for (int q = 0; q < 9; ++q) {
            ...
            for (int dxx = -1; dxx <= 1; ++dxx) {
              ...
              for (u32 sj = 0; sj < cap; ++sj) {
                ...
                fx = Fmaf(ddx, fpair, fx);
                fy = Fmaf(ddy, fpair, fy);
                fz = Fmaf(ddz, fpair, fz);
                if (eflag) {
    ...
    if (eflag) {
      ...
      TeamReduceAdd(team, red, vals, 3, acc);
    }
  }
};

/** K2c: build the Verlet list for this rank's rows. */
struct BuildListFunctor {
  Decomp d;
  const float *x;
  int *nl;
  float rlist;
 ...
 * this rank. Atoms leaving the slab are PACKED into a per-direction outbox
 * and unpacked by the receiving rank after an MPI exchange, exactly as in the
 * MPI sibling: Kokkos gives a portable atomic, not a remote one.
 *
 * Flat RangePolicy here rather than a team: there is no cooperation, only a
 * per-slot atomic claim.
 */
struct RebinFunctor {
  Decomp d;
  float *x;
  ...
  KOKKOS_INLINE_FUNCTION void operator()(const u64 s) const {
    const u32 nb = d.nb, cap = d.cap;
    ...
    if (rel < d.nplanes) {                    // stays here
      ...
      const u32 slot = Kokkos::atomic_fetch_add(&bincnt[lbin], 1u);
      if (slot >= cap) {
        *err = 1;
        dest_pe[s] = -1;
        return;
    ...
    // skin rule), so the destination is always an ADJACENT rank.
    const bool down = (bz == (d.z0 + d.nb - 1) % d.nb);
    u32 *const nout = down ? nout_lo : nout_hi;
    float *const out = down ? out_lo : out_hi;
    const u32 idx = Kokkos::atomic_fetch_add(nout, 1u);
    if (idx >= out_cap) {
      Kokkos::atomic_add(&ctr[kCtrMigrantOverflow], 1ull);
      *err = 2;
      dest_pe[s] = -1;
      return;
    }
...
struct UnpackFunctor {
  ...
  KOKKOS_INLINE_FUNCTION void operator()(const u32 i) const {
    const u32 nb = d.nb, cap = d.cap;
...
struct MDIntegrateFunctor {
  ...
  KOKKOS_INLINE_FUNCTION void operator()(const u64 s) const {
    ...
    const float vx = Fmaf(half, f[e + 0], v[e + 0]);
    const float vy = Fmaf(half, f[e + 1], v[e + 1]);
    const float vz = Fmaf(half, f[e + 2], v[e + 2]);
    v[e + 0] = vx; v[e + 1] = vy; v[e + 2] = vz;
    if (drift) {
      x[e + 0] = Fmaf(dt, vx, x[e + 0]);
      x[e + 1] = Fmaf(dt, vy, x[e + 1]);
      x[e + 2] = Fmaf(dt, vz, x[e + 2]);
    }
  }
};
...
struct IntegrateFunctor {
  ...
  KOKKOS_INLINE_FUNCTION void operator()(const u64 s) const {
    ...
    const u64 e = s * kStride;
    if (x[e + 3] < 0.0f) return;
    const float vx = Fmaf(half, gx, v[e + 0]);
    const float vy = Fmaf(half, gy, v[e + 1]);
    const float vz = Fmaf(half, gz, v[e + 2]);
    v[e + 0] = vx; v[e + 1] = vy; v[e + 2] = vz;
    if (drift) {
      x[e + 0] = Fmaf(dt, vx, x[e + 0]);
      x[e + 1] = Fmaf(dt, vy, x[e + 1]);
...
struct ThermoFunctor {
  ...
  KOKKOS_INLINE_FUNCTION void operator()(const Team &team) const {
    ...
    for (u64 s = static_cast<u64>(team.league_rank()) * ts + tr;
         s < local_slots;
         s += static_cast<u64>(team.league_size()) * ts) {
      const u64 e = s * kStride;
    ...
    TeamReduceAdd(team, red, vals, 4, out);
  }
};

// ---------------------------------------------------------------------------
// Launch helpers. `blocks`/`threads` keep their CLI meaning -- they become the
// league size and the team size -- so a run of this bench and a run of the MPI
// sibling with the same flags really do describe the same decomposition.
// ---------------------------------------------------------------------------

static TeamPol TeamGrid(u32 blocks, u32 threads, size_t scratch_bytes) {
  TeamPol p(static_cast<int>(blocks), static_cast<int>(threads));
  if (scratch_bytes > 0) {
    p = p.set_scratch_size(0, Kokkos::PerTeam(scratch_bytes));
  }
  return p;
}

/** Flat range over every local slot; the CUDA siblings grid-strided this. */
using FlatPol = Kokkos::RangePolicy<ExecSpace, u64>;
using FlatPolU32 = Kokkos::RangePolicy<ExecSpace, u32>;

// ---------------------------------------------------------------------------
// Device memory, through Kokkos rather than the CUDA runtime.
//
// Raw pointers, not Views, on purpose: the kernels above index a bin-major
// padded layout with hand-computed offsets that a View's indexing would only
// obscure, and keeping them raw is what lets the pair loops stay LINE FOR LINE
// identical to the siblings -- which is the whole basis of the cross-substrate
// gates. Allocation, deallocation, transfer and fencing all still go through
// Kokkos, so the backend is still Kokkos' choice.
// ---------------------------------------------------------------------------

// Byte-oriented on purpose: the driver below allocates in `n * sizeof(T)`
// terms exactly as the CUDA sibling does, so a byte interface lets the host
// code stay textually identical to it. Views are unmanaged char views; a
// char-wise deep_copy is a plain memcpy in every Kokkos backend.
using DevChar =
    Kokkos::View<char *, ExecSpace::memory_space, Kokkos::MemoryUnmanaged>;
using DevCharC = Kokkos::View<const char *, ExecSpace::memory_space,
                              Kokkos::MemoryUnmanaged>;
using HostChar = Kokkos::View<char *, Kokkos::HostSpace, Kokkos::MemoryUnmanaged>;
using HostCharC =
    Kokkos::View<const char *, Kokkos::HostSpace, Kokkos::MemoryUnmanaged>;

static void *DevAllocBytes(size_t bytes) {
  void *p = Kokkos::kokkos_malloc<ExecSpace::memory_space>(bytes);
  if (p == nullptr && bytes > 0) {
    std::fprintf(stderr, "Kokkos device allocation of %zu bytes failed\n",
                 bytes);
    std::abort();
  }
  return p;
...
static void DevFreeBytes(void *p) {
  if (p != nullptr) Kokkos::kokkos_free<ExecSpace::memory_space>(p);
}

/** `ptr = <its own type>(DevAllocBytes(bytes))` -- no type argument needed, so
 *  every cudaMalloc site converts by pattern rather than by hand. */
#define MD_DEV_ALLOC(ptr, bytes)                                             \
  (ptr) = static_cast<                                                       \
      std::remove_reference<decltype(*(ptr))>::type *>(DevAllocBytes(bytes))

#define MD_HOST_ALLOC(ptr, bytes)                                            \
  (ptr) = static_cast<std::remove_reference<decltype(*(ptr))>::type *>(       \
      std::malloc(bytes))

static void HostFree(void *p) { std::free(p); }

static void CopyBytesToDev(void *dst, const void *src, size_t bytes) {
  Kokkos::deep_copy(DevChar(static_cast<char *>(dst), bytes),
                    HostCharC(static_cast<const char *>(src), bytes));
}

static void CopyBytesToHost(void *dst, const void *src, size_t bytes) {
  Kokkos::deep_copy(HostChar(static_cast<char *>(dst), bytes),
                    DevCharC(static_cast<const char *>(src), bytes));
}

static void DevZeroBytes(void *p, size_t bytes) {
  Kokkos::deep_copy(DevChar(static_cast<char *>(p), bytes), char{0});
}
...
static int RunBench(int argc, char **argv) {
  ...
  // Kokkos picks the device, and is told to do it per MPI rank -- the
  // cudaSetDevice(mype % ndev) the CUDA siblings do by hand.
  {
    Kokkos::InitializationSettings s;
    s.set_map_device_id_by("mpi_rank");
    Kokkos::initialize(s);
  }
  // Device selection is Kokkos': --kokkos-map-device-id-by=mpi_rank is
  // requested in the InitializationSettings below.
  ...
  if (root) {
    std::printf(
        "eternia-MD / Kokkos baseline (two-sided, host-staged halo)\n"
        "  atoms=%llu box=%.4f bins=%u^3 cap=%u  ranks=%d (planes/rank=%u)\n"
        "  blocks=%u threads=%u rowchunk=%u list=%s halo=%s backend=%s\n"
  ...
  MD_DEV_ALLOC(dx_ext, ext_elems * sizeof(float));
  MD_DEV_ALLOC(dv, local_elems * sizeof(float));
  MD_DEV_ALLOC(df, local_elems * sizeof(float));
  float *dx = dx_ext + own_off;
  {
    ...
    CopyBytesToDev(dx_ext, sx.data(), ext_elems * sizeof(float));
    CopyBytesToDev(dv, sv.data(), local_elems * sizeof(float));
  }
  DevZeroBytes(df, local_elems * sizeof(float));
  ...
  MD_DEV_ALLOC(d_ctr, kCtrNumCtrs * sizeof(unsigned long long));
  DevZeroBytes(d_ctr, kCtrNumCtrs * sizeof(unsigned long long));
  double *d_thermo = nullptr;
  MD_DEV_ALLOC(d_thermo, 4 * sizeof(double));
  ...
  MD_HOST_ALLOC(h_send_lo, g.plane_elems * sizeof(float));
  MD_HOST_ALLOC(h_send_hi, g.plane_elems * sizeof(float));
  MD_HOST_ALLOC(h_recv_lo, g.plane_elems * sizeof(float));
  MD_HOST_ALLOC(h_recv_hi, g.plane_elems * sizeof(float));
  ...
  auto exchange_halo = [&]() {
    ...
    CopyBytesToHost(h_send_hi, first, pb);
    CopyBytesToHost(h_send_lo, last, pb);
    ...
    CopyBytesToDev(dx_ext, h_recv_lo, pb);
    CopyBytesToDev(dx_ext + static_cast<u64>(mynplanes + 1) * g.plane_elems, h_recv_hi, pb);
    t_halo += NowMs() - t;
    halo_bytes += 2.0 * pb;
    ++n_halo;
  };
  ...
  if (a.md) {
    ...
    MD_DEV_ALLOC(dx2_ext, ext_elems * sizeof(float));
    MD_DEV_ALLOC(dv2, local_elems * sizeof(float));
    ...
    MD_DEV_ALLOC(d_bincnt, static_cast<u64>(mynplanes) * g.nb * g.nb * sizeof(u32));
    MD_DEV_ALLOC(d_dest_pe, local_slots * sizeof(int));
    MD_DEV_ALLOC(d_dest_slot, local_slots * sizeof(u32));
    MD_DEV_ALLOC(d_cnt, local_slots * sizeof(u32));
    MD_DEV_ALLOC(d_err, sizeof(int));
    MD_DEV_ALLOC(d_acc, 3 * sizeof(double));
    MD_DEV_ALLOC(d_out_lo, static_cast<u64>(out_cap) * 9 * sizeof(float));
    MD_DEV_ALLOC(d_out_hi, static_cast<u64>(out_cap) * 9 * sizeof(float));
    MD_DEV_ALLOC(d_in, static_cast<u64>(out_cap) * 9 * sizeof(float));
    MD_DEV_ALLOC(d_nout_lo, sizeof(u32));
    MD_DEV_ALLOC(d_nout_hi, sizeof(u32));
    if (a.use_list) {
      MD_DEV_ALLOC(d_nl, nl_elems * sizeof(int));
      DevZeroBytes(d_nl, nl_elems * sizeof(int));
    }
    ...
    auto build_list = [&]() -> bool {
      const double t = NowMs();
      DevZeroBytes(d_err, sizeof(int));
      Kokkos::parallel_for(
          "md_build_list", TeamGrid(a.blocks, a.threads, smem_build),
          BuildListFunctor{d, dx_ext, d_nl, frlist, a.maxneigh, d_cnt, d_err,
                           g.row_elems, g.plane_elems, a.rowchunk, fbox});
      Kokkos::fence();
      int err = 0;
      CopyBytesToHost(&err, d_err, sizeof(int));
      double e = err;
      sum_doubles(&e, 1);
      t_build += NowMs() - t;
      if (e != 0.0) {
    ...
    auto force = [&](int eflag) {
      const double t = NowMs();
      if (eflag) DevZeroBytes(d_acc, 3 * sizeof(double));
      if (a.use_list) {
        Kokkos::parallel_for(
            "md_list_force", TeamGrid(a.blocks, a.threads, smem_force),
            ListForceFunctor{d, dx_ext, df, d_nl, d_cnt, fbox, fcut,
                             a.maxneigh, eflag, d_acc, g.row_elems,
                             g.plane_elems, a.rowchunk, nocompute});
      } else {
        Kokkos::parallel_for(
            "md_force", TeamGrid(a.blocks, a.threads, smem_force),
            ForceFunctor{d, dx_ext, df, fbox, fcut, eflag, d_acc,
                         g.row_elems, g.plane_elems, a.rowchunk});
      }
      Kokkos::fence();
      if (eflag) {
        CopyBytesToHost(acc, d_acc, 3 * sizeof(double));
        sum_doubles(acc, 3);
      }
      t_force += NowMs() - t;
    };
    auto kick = [&](int drift) {
      const double t = NowMs();
      Kokkos::parallel_for("md_integrate", FlatPol(0, local_slots),
                           MDIntegrateFunctor{dx, dv, df, fdt, drift});
      Kokkos::fence();
      t_kick += NowMs() - t;
    };
    auto thermo_ke = [&]() -> double {
      DevZeroBytes(d_thermo, 4 * sizeof(double));
      Kokkos::parallel_for(
          "md_thermo", TeamGrid(a.blocks, a.threads, smem_thermo),
          ThermoFunctor{dx, dv, d_thermo, local_slots});
      Kokkos::fence();
      double t4[4];
      CopyBytesToHost(t4, d_thermo, sizeof(t4));
      sum_doubles(t4, 4);
      return t4[0];
    };
    /**
    ...
    auto resort = [&]() -> bool {
      const double t = NowMs();
      DevZeroBytes(d_bincnt, static_cast<u64>(mynplanes) * g.nb * g.nb * sizeof(u32));
      DevZeroBytes(d_err, sizeof(int));
      DevZeroBytes(d_nout_lo, sizeof(u32));
      DevZeroBytes(d_nout_hi, sizeof(u32));
      Kokkos::parallel_for("md_sentinel_x", FlatPol(0, local_slots),
                           SentinelFunctor{dx2});
      Kokkos::parallel_for("md_sentinel_v", FlatPol(0, local_slots),
                           SentinelFunctor{dv2});
      Kokkos::fence();
      Kokkos::parallel_for(
          "md_rebin", FlatPol(0, local_slots),
          RebinFunctor{d, dx, dv, fbox, d_bincnt, d_dest_pe, d_dest_slot,
                       d_err, d_out_lo, d_out_hi, d_nout_lo, d_nout_hi,
                       out_cap, d_ctr});
      Kokkos::fence();
      int err = 0;
      CopyBytesToHost(&err, d_err, sizeof(int));
      ...
      Kokkos::parallel_for(
          "md_scatter_x", FlatPol(0, local_slots),
          ScatterFunctor{dx, dx, dx2, d_dest_pe, d_dest_slot, 1});
      Kokkos::parallel_for(
          "md_scatter_v", FlatPol(0, local_slots),
          ScatterFunctor{dv, dx, dv2, d_dest_pe, d_dest_slot, 0});

      if (npes > 1) {
        u32 n_lo = 0, n_hi = 0;
        CopyBytesToHost(&n_lo, d_nout_lo, sizeof(u32));
        CopyBytesToHost(&n_hi, d_nout_hi, sizeof(u32));
        ...
        if (n_lo) {
          CopyBytesToHost(h_mig.data(), d_out_lo, static_cast<u64>(n_lo) * 9 * sizeof(float));
        }
        ...
        if (n_hi) {
          CopyBytesToHost(h_mig.data(), d_out_hi, static_cast<u64>(n_hi) * 9 * sizeof(float));
        }
        ...
        if (nrecv) {
          CopyBytesToDev(d_in, h_mig_in.data(), static_cast<u64>(nrecv) * 9 * sizeof(float));
          Kokkos::parallel_for(
              "md_unpack", FlatPolU32(0, nrecv),
              UnpackFunctor{d, d_in, d_bincnt, dx2, dv2, d_err});
        }
      }
      Kokkos::fence();
      CopyBytesToHost(&err, d_err, sizeof(int));
    ...
    if (a.ckpt != 0) {
      MD_HOST_ALLOC(h_ckpt, ckpt_elems * sizeof(float));
    }
    auto checkpoint = [&]() {
      const double _t = NowMs();
      CopyBytesToHost(h_ckpt, dx, local_elems * sizeof(float));
      CopyBytesToHost(h_ckpt + local_elems, dv, local_elems * sizeof(float));
      t_ckpt += NowMs() - _t;
    ...
    if (h_ckpt != nullptr) HostFree(h_ckpt);

    unsigned long long ctr[kCtrNumCtrs] = {0};
    CopyBytesToHost(ctr, d_ctr, sizeof(ctr));
    ...
    if (root) {
      ...
      std::printf(
          ...
          "  substrate: Kokkos kernels over two-sided MPI, host-staged "
          "(D2H -> MPI -> H2D); "
  ...
  for (u64 step = 0; step < a.steps; ++step) {
    Kokkos::parallel_for(
        "md_ballistic_drift", FlatPol(0, local_slots),
        IntegrateFunctor{dx, dv, fdt, fgx, fgy, fgz, 1});
    Kokkos::parallel_for(
        "md_ballistic_kick", FlatPol(0, local_slots),
        IntegrateFunctor{dx, dv, fdt, fgx, fgy, fgz, 0});
  }
  Kokkos::fence();
  MPI_Barrier(MPI_COMM_WORLD);
  const double run_ms = NowMs() - t0;
  ...
  Kokkos::parallel_for(
          ...
          ThermoFunctor{dx, dv, d_thermo, local_slots});
  Kokkos::fence();
  CopyBytesToHost(thermo, d_thermo, sizeof(thermo));
  sum_doubles(thermo, 4);

  int rc = 0;
  if (a.gate) {
    std::vector<float> gx_out(local_elems), gv_out(local_elems);
    CopyBytesToHost(gx_out.data(), dx, local_elems * sizeof(float));
    CopyBytesToHost(gv_out.data(), dv, local_elems * sizeof(float));
...
int main(int argc, char **argv) {
  const int rc = RunBench(argc, argv);
  // RunBench can bail out before Kokkos was ever initialised (bad arguments),
  // and finalizing an uninitialised Kokkos aborts.
