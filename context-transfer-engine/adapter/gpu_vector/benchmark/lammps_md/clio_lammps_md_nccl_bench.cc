...
/**
 * eternia-MD, NCCL edition: the same melt deck and the same kernels as its
 * MPI and NVSHMEM siblings, with the transport changed once more.
 *
 * The three of them isolate two independent properties of a transport, and
 * NCCL is here because it is the one that separates them:
 *
 *                     |  two-sided?  |  reads GPU memory directly?
 *   md_mpi_bench      |     yes      |  NO -- D2H, MPI, H2D on every halo
 *   md_nccl_bench     |     yes      |  YES -- ncclSend/Recv on device ptrs
 *   md_nvshmem_bench  |     no       |  YES -- and from inside the kernel
 *
 * So MPI vs NCCL prices the HOST BOUNCE with two-sidedness held fixed, and
 * NCCL vs NVSHMEM prices ONE-SIDEDNESS with GPU-residency held fixed. That
 * is the whole reason to write a third one rather than assume the answer.
 *
 * Everything else is unchanged and copied verbatim: float precision,
 * bin-major padded layout, ghost-free minimum image, full list / newton off,
 * the transposed-padded Verlet list, the chunked row decomposition, and all
 * four gates. As with the MPI sibling, each rank holds an EXTENDED slab
 * [halo_lo | owned planes | halo_hi] and the kernels contain no
 * communication at all -- the difference is only how the halo gets there.
 *
 * ONE GPU PER RANK IS A HARD NCCL REQUIREMENT. ncclCommInitRank rejects two
 * ranks that name the same device ("Duplicate GPU"), so unlike the other two
 * benches this one CANNOT be exercised multi-rank on a single-GPU host: it
 * runs there at one rank (where no halo is needed at all) and needs real
 * multi-GPU hardware for anything else. That is a property of NCCL, not a
 * shortcut taken here, and the run refuses with a clear message rather than
 * deadlocking.
 *
 * Bootstrap is the standard one: MPI discovers the ranks, rank 0 broadcasts
 * an ncclUniqueId, everyone calls ncclCommInitRank. MPI also carries the
 * migrant COUNTS (4 bytes, and NCCL needs both sides to agree on a size
 * before it can move the payload); every byte of simulation state moves
 * through NCCL, on device pointers.
 *
 * Run recipe:
 *   mpirun -n 4 clio_md_nccl_bench --md --lattice 40 --steps 100 --rebin 10 \
 *       --temp 3.0 --cap 48 --blocks 529 --threads 128 --rowchunk 1 \
 *       --drift-tol 5e-3          # 4 GPUs, one per rank
 */
...
#define MD_NCCL_CHECK(call)                                                  \
  do {                                                                       \
    const ncclResult_t _r = (call);                                          \
    if (_r != ncclSuccess) {                                                 \
...
int main(int argc, char **argv) {
  ...
  if (root) {
    std::printf(
        "eternia-MD / NCCL baseline (two-sided, GPU-resident halo)\n"
  ...
  // NO staging buffers. That absence IS the difference from the MPI sibling:
  // NCCL takes device pointers, so the halo never touches host memory.
  ...
  const bool need_halo = (npes > 1) || a.force_halo;
  {
    ...
    MD_NCCL_CHECK(ncclCommInitRank(&nccl_comm, npes, nccl_id, mype));
    MD_CUDA_CHECK(cudaStreamCreate(&nccl_stream));
  }

  /**
   * Exchange the two boundary planes of x, straight out of and into the
   * extended slab. My FIRST owned plane is what the down neighbour needs as
   * its halo_hi; my LAST is what the up neighbour needs as its halo_lo.
   *
   * All four point-to-point operations go in ONE NCCL group: outside a
   * group, a send that blocks waiting for its matching receive would
   * deadlock a ring of ranks that are all sending first. The group is also
   * what lets NCCL schedule the two directions concurrently.
   */
  auto exchange_halo = [&]() {
    if (!need_halo) return;
    const double t = NowMs();
    const u64 pb = g.plane_elems * sizeof(float);
    const int n = static_cast<int>(g.plane_elems);
    float *const first = dx;
    float *const last = dx + static_cast<u64>(mynplanes - 1) * g.plane_elems;
    float *const halo_lo = dx_ext;
    float *const halo_hi =
        dx_ext + static_cast<u64>(mynplanes + 1) * g.plane_elems;
    MD_NCCL_CHECK(ncclGroupStart());
    MD_NCCL_CHECK(ncclSend(last, n, ncclFloat, rank_up, nccl_comm,
                           nccl_stream));
    MD_NCCL_CHECK(ncclRecv(halo_lo, n, ncclFloat, rank_down, nccl_comm,
                           nccl_stream));
    MD_NCCL_CHECK(ncclSend(first, n, ncclFloat, rank_down, nccl_comm,
                           nccl_stream));
    MD_NCCL_CHECK(ncclRecv(halo_hi, n, ncclFloat, rank_up, nccl_comm,
                           nccl_stream));
    MD_NCCL_CHECK(ncclGroupEnd());
    MD_CUDA_CHECK(cudaStreamSynchronize(nccl_stream));
    t_halo += NowMs() - t;
    halo_bytes += 2.0 * pb;
    ++n_halo;
  };
  ...
  if (a.md) {
    ...
    auto resort = [&]() -> bool {
      ...
      if (npes > 1) {
        ...
        // The PAYLOAD never touches host memory: the outboxes are device
        // buffers and so is the inbox, so NCCL moves migrants GPU-to-GPU.
        // Only the counts above went through MPI, and they had to: NCCL
        // requires both sides to agree on a size before the transfer, and
        // it has no mechanism for discovering one.
        //
        // Zero-length operations are skipped rather than issued. NCCL
        // tolerates them, but a rank that migrates nothing this rebuild
        // should not enter the group at all -- and at low temperature that
        // is the common case.
        MD_NCCL_CHECK(ncclGroupStart());
        if (n_lo) {
          MD_NCCL_CHECK(ncclSend(d_out_lo, static_cast<int>(n_lo * 9),
                                 ncclFloat, rank_down, nccl_comm,
                                 nccl_stream));
        }
        if (r_from_up) {
          MD_NCCL_CHECK(ncclRecv(d_in, static_cast<int>(r_from_up * 9),
                                 ncclFloat, rank_up, nccl_comm, nccl_stream));
        }
        if (n_hi) {
          MD_NCCL_CHECK(ncclSend(d_out_hi, static_cast<int>(n_hi * 9),
                                 ncclFloat, rank_up, nccl_comm, nccl_stream));
        }
        if (r_from_down) {
          MD_NCCL_CHECK(ncclRecv(d_in + static_cast<u64>(r_from_up) * 9,
                                 static_cast<int>(r_from_down * 9), ncclFloat,
                                 rank_down, nccl_comm, nccl_stream));
        }
        MD_NCCL_CHECK(ncclGroupEnd());
        MD_CUDA_CHECK(cudaStreamSynchronize(nccl_stream));
    ...
    if (root) {
      ...
      std::printf(
          ...
          "  substrate: two-sided NCCL on device pointers; the halo never "
          "enters host memory, which is the single difference from the MPI "
          "sibling (same kernels, same two-sidedness)\n",
