...
/* Per-lane yield frame. The macro edition does NOT get the 256 B a flat
 * hand-packed kernel would need: the device API nests (-> MFetch ->
 * MBeginFetch), each level pushes a frame, and MBeginFetch alone carries
 * two kMaxFetchRanges u64 arrays. Keep the coroutine edition's 4 KiB. */
 ...
 */
#include "grayscott_macros_kernels.h"
#include "grayscott_macros_launch.h"

namespace gs = clio::gv_bench::grayscott_macros;
