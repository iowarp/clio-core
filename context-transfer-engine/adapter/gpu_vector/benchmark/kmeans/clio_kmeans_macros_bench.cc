...
/**
 * kmeans paged benchmark, MACRO-form edition.
 *
 * Byte-for-byte the driver of clio_kmeans_paged_bench.cc -- same problem, same
 * geometry, same gates -- differing only in which launch seam it binds:
 * kmeans_macros_launch.h instead of kmeans_launch.h. The device side behind
 * that seam expresses suspension with yield_stack.h's macros rather than C++20
 * coroutines.
 *
 * WHY IT EXISTS. clang cannot compile a device coroutine for SPIR-V: its
 * EmitCoroutineBody builds the coroutine-frame PHI as ptr addrspace(0)
 * (CGCoroutine.cpp) and the whole llvm.coro.* family is declared llvm_ptr_ty
 * rather than llvm_anyptr_ty, while spir64's datalayout carries -G1. Verified
 * on Aurora oneAPI 2025.2 and 2025.3.2 and on intel/llvm nightly-2026-09-18
 * (clang 24); no optimisation level or flag avoids it, and hand-rolling with
 * __builtin_coro_* fails the same way one layer down. NVPTX is unaffected only
 * because its default address space is 0 -- which is why the coroutine edition
 * remains the CUDA path and nothing about it changes.
 *
 * The two editions are meant to be compared directly: any divergence in the
 * gates between them is a defect in the macro port, not in the workload.
 */
/**
...
/* Per-lane yield frame.
 *
 * The macro edition does NOT get the 256 B a flat hand-packed kernel would
 * need: the device API nests (Gather/Seed -> MFetch -> MBeginFetch), each
 * level pushes its own frame, and MBeginFetch alone carries two
 * kMaxFetchRanges u64 arrays. Keep the coroutine edition's 4 KiB. */
...
namespace kb = clio::gv_bench::kmeans_macros;
using kb::PointVal;
