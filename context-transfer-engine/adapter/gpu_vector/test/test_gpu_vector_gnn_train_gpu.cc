...
__global__ void GnnGatherKernel(clio::run::IpcManagerGpuInfo info,
  ...
  v.read_range(lo, hi, [scratch, lo](clio::run::u64 i, float val) { scratch[i - lo] = val; });
  (void)g_ipc_manager;
}
...
TEST_CASE("gpu_vector: GNN training over a compressed/streamed feature matrix "
  ...
  clio::run::u64 N0 = 0, E0 = 0; int F = 128, C = 40;
  ...
  std::fprintf(stderr,
      ...
      (unsigned long long)(dataset_bytes >> 20), (unsigned long long)N, F, F, H, C,
  ...
  { std::ifstream f(dir + "/agg_features.f32", std::ios::binary); REQUIRE(f.good());
    f.read((char *)A0.data(), (std::streamsize)(A0.size() * sizeof(float))); REQUIRE((bool)f); }
  std::vector<long long> lab0(N0);
  { std::ifstream f(dir + "/labels.i64", std::ios::binary); REQUIRE(f.good());
    f.read((char *)lab0.data(), (std::streamsize)(N0 * sizeof(long long))); REQUIRE((bool)f); }
  ...
  for (int e = 0; e < epochs; ++e) {
    ...
    run_epoch([&](clio::run::u64 wi, clio::run::u64 first, clio::run::u64 nn) {
      vec.PrefetchPagesSync(wi, window, 0);
