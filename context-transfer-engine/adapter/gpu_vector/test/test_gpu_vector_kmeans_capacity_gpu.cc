...
TEST_CASE("gpu_vector: k-means capacity comparison (traditional in-core vs "
  ...
  double trad_runtime = -1.0;
  {
    ...
    if (alloc != cudaSuccess) {
      ...
      double dt = NowSec() - t0;
      trad_status = "OK"; trad_runtime = dt;
      std::fprintf(stderr,
          "[CAP] TRADITIONAL: OK, ran in %.2fs, final inertia=%.1f (but pinned "
          "%lluMiB of HBM).\n", dt, prev, (unsigned long long)(dataset_bytes >> 20));
      cudaFree(d_all);
    }
  }
  ...
  std::fprintf(stderr,
      ...
      "window=%lluKiB (%.0fx smaller than the %lluMiB dataset).\n",
      km_dt, prev,
      (unsigned long long)(win_elems * sizeof(float) >> 10),
      (double)dataset_bytes / (double)(win_elems * sizeof(float)),
      (unsigned long long)(dataset_bytes >> 20));
  std::fprintf(stderr, "====================================================\n");
  ...
  {
    const clio::run::u64 peak_win_mib =
        (win_elems * sizeof(float)) >> 20 ? (win_elems * sizeof(float)) >> 20 : 0;
    char row[512];
    std::snprintf(row, sizeof(row),
        "%llu,%llu,%u,%llu,%s,%d,%s,%.2f,%.2f,%.2f,%llu,%.2f,%llu,%llu,%llu",
        ...
        (unsigned long long)peak_win_mib);             // eternia_peak_gpu_mib
    const char *hdr =
        "dataset_mib,page_kib,window_pages,window_mib,mode,iters,traditional_status,"
        "traditional_runtime_s,eternia_store_s,eternia_km_runtime_s,eternia_stored_mib,"
        "eternia_ratio,hbm_used_mib,dram_used_mib,eternia_peak_gpu_mib";
