...
| 25 | Performance comparison | **PARTIAL — inherited like-for-like numbers; no new measurement** |
...
| **D15-1** | 15 | **HIGH (interop)** | Blob container formats differ entirely — magic "GPUC"/64 B fixed vs "CTEC"/24 B + optional 32 B extension, different field layout and algorithm-id placement. Neither system can read the other's blobs. **By design**, not a defect; but Phase 18 cross-compatibility must be re-scoped to payload bytes rather than blob interchange. |
| **D21-1** | 21 | **HIGH — ARCHITECTURAL, root cause of VOL-1/VOL-3** | **Native records compression IN-BAND as a real HDF5 filter; CLIO records it out-of-band and has no filter at all.** Native registers `H5Z_FILTER_GPUCOMPRESS` (id 305) via `H5Zregister` and installs it with `H5Pset_filter` (`src/hdf5/H5Zgpucompress.c:68`, `:400`, `:443`, `:471`), so the filter lives in the dataset's DCPL inside the file. On reopen HDF5 itself sees it and invokes decompression — persistence is HDF5's job and is automatic. CLIO has **no `H5Zregister`/`H5Pset_filter` anywhere** in `adapter/hdf5_vol/`; it records compression in a CLIO-side stamp blob with a tag coherence check, and writes the plain data to native HDF5 as well.<br>This single choice is the root cause of three separately-filed findings: **VOL-3** (reopen finds no stamp, drops the tag, serves the uncompressed copy) and **the dataset being stored twice** are inevitable consequences, and **VOL-1**'s full-dataset D2H exists because the plain bytes must also reach HDF5. Fixing VOL-3 by patching the stamp treats a symptom; the design-level options are (a) register an H5Z filter as native does, or (b) persist the stamp so the tag survives reopen. |
...
| **D21-2** | 21 | **HIGH — the two systems do not agree on WHAT A CHUNK IS** | Native compresses the **N-dimensional HDF5 chunk**, gathered on the GPU: `gather_chunk_kernel` (`H5VLgpucompress.cu:1142`) handles arbitrary rank up to `H5S_MAX_RANK=32` with C-order strides, one thread per element, called on the write path (`:1490`, `:2268`) with `scatter_chunk_kernel` (`:1180`) as its read-side twin (`:3342`). CLIO compresses a **flat byte range of the linearised dataset**: the unit is `dataset->file->chunk_size` (`clio_vol.cc:1786`, `:2004`, `:2417`), a plain byte count. It DOES call `H5Pget_chunk` (`:1621`) but the result is used only for selection-alignment classification (`:1645`) and a telemetry string (`:1679-1682`) — never to size or shape the compression unit. `grep -c __global__ clio_vol.cc` → **0**.<br>**Why this is a parity divergence and not just a locality one:** entropy, MAD and mean |2nd derivative| are precisely the three features NeuroPress consumes. An N-D sub-block is spatially local; a flat byte range across a 2-D/3-D dataset spans row boundaries and mixes distant data. So for the same file the network is fed **different feature values**, and may rank a different algorithm — on top of a genuinely different achievable ratio.<br>**This is invisible to every parity test in the suite**, because `ctp_neuropress_dataset_parity` hands both sides THE SAME BYTES. It validated the feature computation; it could never have validated the chunk definition. |
...
| **D23-1** | 23 | **MEDIUM — no back-pressure, no GPU-memory knob** | Native bounds concurrent compressions to a fixed pool: `N_COMP_CTX = 9` `CompContext` slots, and `acquireCompContext` **blocks on a condition variable** until one frees (`gpucompress_pool.cpp:100-112`). The bound is tunable via `GPUCOMPRESS_POOL_SLOTS` (1..9), and the code states the purpose outright — "to cut per-rank GPU memory" (`:32`), i.e. it exists for multi-rank HPC deployment. CLIO has **no explicit bound on the compression path**: no pool, no semaphore, no condition variable; effective concurrency is the worker count, and there is no equivalent knob. Native's per-rank scratch is therefore `slots x per-slot scratch` and CLIO's is `workers x per-chunk scratch` with no ceiling and no way to lower it.<br>Related to the earlier audit's VOL-8 (unbounded staging growth) and a compounding factor for D19-1 while that leak was live. |
...
| **D22-1** | 22 | **MEDIUM — unported feature** | `gpucompress_set_best_mode` has no CLIO counterpart, and it is not a cosmetic knob: `g_best_mode` changes behavior at four sites. It forces exploration on EVERY chunk regardless of `error_pct` — exhaustive search over all 32 configs (`gpucompress_compress.cpp:733`) — and simultaneously SUPPRESSES SGD phase 1 (`:705`) and SGD phase 2 (`:1003`), plus gates VOL behavior (`H5VLgpucompress.cu:1698`). A workload wanting per-chunk exhaustive search cannot express it in CLIO at all. Since CLIO is the integration rather than a wrapper, this is permanently unexpressible, not merely unwired. |
...
| D6-1 | 6 | LOW | No CLIO counterpart to `gpucompress_chunk_diag_t` or its accessors. Observability only — native's own consumers are all tests/benchmarks. Bites at Phase 25/27. |
