...
#                  because Nyx dumps float32 (contrast with LAMMPS float64,
#                  where the model's one shuffle bit means the wrong width)
#   static-zstd-s8 fixed nvcomp-zstd + 8-byte shuffle -- expected to be worse
#                  on float32; included to show the stride actually matters
set -euo pipefail
...
# them. This phase replays files and has no other way to know: hardcoding 0
# meant every Nyx run reported "0 timesteps" in the summary. Older dump
# directories have no manifest, and still report 0.
GEN_STEPS=0 GEN_FRAMES=0
if [ -f "$FIELDS/gen.json" ]; then
  ...
  GEN_FRAMES=$(python3 -c "import json;print(json.load(open('$FIELDS/gen.json')).get('frames',0))" 2>/dev/null || echo 0)
fi
...
{"config":"$CONFIG","tag":"$NAME","rc":$RC,"mode":"$MODE",
 ...
 "error_bound":${EB:-0},"explore_k":$EXPLORE_K,"explore_thresh":$THRESH,"device":"gpu","workload":"nyx-sedov",
