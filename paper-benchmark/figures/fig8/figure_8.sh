...
# plots go to <size>/, beside this script.
#
# Every workload replays a dump, so no simulate time enters the measurement.
...
usage() { sed -n '3,12p' "$0" >&2; cat >&2 <<'U'
  ...
  --no-plot       write trace.csv only; plot later with plot_fig8.py panels
...
OUT=${OUT:-$BENCH/results/figure8/$SIZE}
FIGDIR=$HERE/$SIZE                      # the plots land beside this script
mkdir -p "$OUT"
...
if [ "$SIZE" = smoke ]; then
  ...
  VPIC_FILES=2000 LMP_FILES=2004
  LMP_BOX_2K=60
...
# VPIC dumped every 16 steps (full) or 8 (2k); LAMMPS box 40 (full) or 60 (2k).
VPIC_FIELDS=${VPIC_FIELDS:-$BENCH/vpic/fields}
LAMMPS_FIELDS=${LAMMPS_FIELDS:-$BENCH/lammps/fields}
...
run_wl() {
  ...
  local -a cmd
  case "$wl" in
    vpic)   cmd=( env CLIO_NEUROPRESS_STAGE_H2D=1 "$BENCH/nyx/run_config.sh" "$CONFIG"
                  --fields "$VPIC_FIELDS" --max-files "$VPIC_FILES" "${NP[@]}" --check-bound ) ;;
    nyx)    cmd=( env CLIO_NEUROPRESS_STAGE_H2D=1 "$BENCH/nyx/run_config.sh" "$CONFIG"
                  --fields "$NYX_FIELDS" --max-files "$NYX_FILES" "${NP[@]}" --check-bound ) ;;
    warpx)  cmd=( env CLIO_NEUROPRESS_STAGE_H2D=1 "$BENCH/nyx/run_config.sh" "$CONFIG"
                  --fields "$WARPX_FIELDS" --max-files "$WARPX_FILES" "${NP[@]}" --check-bound ) ;;
    ai)     cmd=( env CLIO_NEUROPRESS_STAGE_H2D=1 "$BENCH/nyx/run_config.sh" "$CONFIG"
                  --fields "$AI_FIELDS" --max-files "$AI_FILES" "${NP[@]}" --check-bound ) ;;
    lammps) cmd=( env CLIO_NEUROPRESS_STAGE_H2D=1 "$BENCH/nyx/run_config.sh" "$CONFIG"
                  --fields "$LAMMPS_FIELDS" --max-files "$LMP_FILES" "${NP[@]}" --check-bound ) ;;
  esac
  ...
  python3 "$HERE/plot_fig8.py" trace "$log" --out "$dir/trace.csv" | sed 's/^/     /'
}
...
if [ "$PLOT" = 1 ] && [ ${#TRACES[@]} -gt 0 ]; then
  echo; python3 "$HERE/plot_fig8.py" panels "${TRACES[@]}" --out "$FIGDIR"
fi
