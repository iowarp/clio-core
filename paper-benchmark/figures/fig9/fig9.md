...
- **Verification** runs once per arm, in rep 1 (it is untimed but a full
  read-back), and covers the **lossless arms only**, compared bit-exact against
  the input. **No lossy arm is checked against its error bound** -- not the
  external codecs (cuSZ, cuSZp3, nvCOMP `-q`, ndzip) and not NeuroPress's lossy
  rungs. This figure reports end-to-end wall clock at a *requested* bound;
  whether a codec honours that bound is an accuracy question, answered by the
  accuracy figures over the same dumps. Every lossy arm runs `--no-verify` and
  leaves the `bound` column **empty, meaning not checked** -- never "checked and
  fine".
