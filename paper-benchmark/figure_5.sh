...
# fig5_<workload>.png, copied to figures/fig5/.
#
...
OUT=${OUT:-$HERE/results/figure5}
mkdir -p "$OUT"
FIGDIR=$HERE/figures/fig5; [ "$SIZE" = smoke ] && FIGDIR=$FIGDIR/smoke
...
run_wl() {
  ...
  case "$wl" in
    vpic)
      warmup=$VPIC_WARMUP
      # From the run directory: the deck writes its `energies` diagnostics to
      # the working directory, which otherwise is wherever this was started.
      step "vpic in situ (${VPIC_NCELL}^3, $VPIC_STEPS steps, dump every $VPIC_INT)" "$dir/console.log" \
        env -C "$dir" "${env_kv[@]}" "$HERE/vpic/run_config_insitu.sh" "$CONFIG" \
          --ncell "$VPIC_NCELL" --nppc "$VPIC_NPPC" --steps "$VPIC_STEPS" --int "$VPIC_INT" \
          --chunk "$CHUNK" "${EXPLORE[@]}" --verify --results "$dir" --tag "$wl" ;;
    nyx|warpx)
      local fields="$dir/fields"
      if [ "$wl" = nyx ]; then
        step "nyx simulation dumps fields (${NYX_NCELL}^3, $NYX_STEPS steps)" "$dir/gen.log" \
          "$HERE/nyx/gen_fields.sh" --ncell "$NYX_NCELL" --steps "$NYX_STEPS" \
        ...
        step "warpx simulation dumps fields ($WARPX_NCELL, $WARPX_STEPS steps)" "$dir/gen.log" \
          "$HERE/analysis/validate/warpx_gen_fields.sh" --ncell "$WARPX_NCELL" \
      ...
      step "$wl replay through Clio" "$dir/console.log" \
        env "${env_kv[@]}" CLIO_NEUROPRESS_STAGE_H2D=1 "$HERE/nyx/run_config.sh" "$CONFIG" \
          --fields "$fields" --chunk "$CHUNK" "${EXPLORE[@]}" --results "$dir" --tag "$wl" ;;
    lammps)
      warmup=$LMP_WARMUP
      step "lammps in situ (box $LMP_BOX, $LMP_STEPS steps, dump every $LMP_GAP)" "$dir/console.log" \
        env "${env_kv[@]}" "$HERE/lammps/run_config.sh" "$CONFIG" \
          --box "$LMP_BOX" --steps "$LMP_STEPS" --gap "$LMP_GAP" --chunk "$CHUNK" \
          "${EXPLORE[@]}" --require-device --results "$dir" --tag "$wl" ;;
  esac
  local rc=$?
  [ "$DRY" = 1 ] && return 0
  ...
  python3 "$HERE/plot/fig5_timesteps.py" "$phase" --workload "${NAME[$wl]}" \
    --warmup-step "$warmup" --out "$ts" | sed 's/^/     /'
  python3 "$HERE/plot/plot_fig5.py" --timesteps "$ts" --no-defaults \
